...
<ol class="breadcrumb">
  ...
  <li><a href="{{ route('admin.coupon.index') }}"><i class="fa fa-sliders" aria-hidden="true"></i> Quản Lý Danh Mục</a></li>
  <li class="active">Chỉnh Sửa Danh Mục</li>
</ol>
@endsection

@section('content')
<form id="producer-form" action="{{ route('admin.producer.update', ['id' => $producer?->id]) }}" method="POST" accept-charset="utf-8">
  @csrf
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="form-group">
            ...
            <input type="text" name="name" class="form-control" id="name" placeholder="Tên" value="{{ old('name') ?: $producer?->name }}" autocomplete="off">
            @error('name')
