...
@section('content')
    <form id="createproduct-form" action="{{ route('admin.products_.store') }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('POST')
        <div class="row">
            <div class="col-lg-8">
...
@push('script')
    ...
    {{-- <script>
        $(document).ready(function() {
            let variantIndex = 0; // Chỉ số biến thể

            // Sự kiện click nút "Thêm biến thể"
            $('#addVariant').click(function() {
                let selectedValues = $('#attributeSelect').val(); // Lấy giá trị đã chọn từ select
                ...
                if (selectedValues.length > 0) {
                    let variantHTML = `<div class="variant-group card mb-3" id="variant-${variantIndex}">
                                        <div class="card-body">
                                            <div class="row">`;
                    let ajaxCalls = []; // Mảng để lưu các promise của AJAX

                    selectedValues.forEach(function(variantId) {
                        // Tạo promise cho mỗi AJAX call
                        let ajaxCall = $.ajax({
                            url: '/admin/products_/get-variant-value',
                            type: 'GET',
                            data: {
                                id: variantId
                            }
                        }).then(function(data) {
                            // Tạo phần HTML cho mỗi thuộc tính
                            let attributeHTML =
                                `
                                <div class="form-group col-md-4 mt-2">
                                    <label for="variantValues-${variantId}-${variantIndex}">Chọn giá trị:</label>
                                    <select class="form-control" name="variant_values[${variantIndex}][${variantId}]" id="variantValues-${variantId}-${variantIndex}">`;

                            // Duyệt qua các giá trị nhận được và tạo option cho select
                            ...
                            return attributeHTML; // Trả về phần HTML cho thuộc tính
                        });

                        ajaxCalls.push(ajaxCall); // Thêm promise vào mảng
                    });

                    // Đợi tất cả các yêu cầu AJAX hoàn thành
                    Promise.all(ajaxCalls).then(function(attributeHTMLArray) {
                        // Thêm tất cả các thuộc tính vào variantHTML
                        attributeHTMLArray.forEach(function(attributeHTML) {
                            variantHTML += attributeHTML;
                        });

                        // Thêm các trường Số lượng và Giá
                        variantHTML += `
                            <div class="form-group col-md-4 mt-2">
                                <label for="qty-${variantIndex}">Số lượng:</label>
                                <input type="number" name="variants[${variantIndex}][qty]" id="qty-${variantIndex}" class="form-control" required>
                            </div>
                            <div class="form-group col-md-4 mt-2">
                                <label for="price_variant-${variantIndex}">Giá biến thể:</label>
                                <div class="input-group">
                                    <input type="text" name="variants[${variantIndex}][price_variant]" id="price_variant-${variantIndex}" class="form-control" required>
                                    <span class="input-group-text">VNĐ</span>
                                </div>
                            </div>
                            </div>
                        ...
                        $('#variants').append(
                            variantHTML); // Thêm HTML vào phần tử với id "variants"
                        variantIndex++; // Tăng chỉ số biến thể cho lần sau

                        // Gán sự kiện xóa cho nút "Xóa biến thể"
                        $('.remove-variant').off('click').on('click', function() {
                            let variantId = $(this).data('variant-id');
                            $('#' + variantId).remove(); // Xóa nhóm biến thể
                        });
                    });
                }
            });
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.select-variant-type', function() {
            ...
            if (variantTypeId == "0") {
                ...
                $.ajax({
                    url: "/admin/products_/get-variant-value", // Đường dẫn tới route
                    type: "GET",
                    data: {
                        id: variantTypeId
                    },
