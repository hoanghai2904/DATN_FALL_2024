                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        ...
                                                        <select name="values[{{ $attribute->id }}][value][]" class="form-control select2" id="values_{{ $attribute->id }}" multiple="multiple" required>
                                                            @foreach($attribute->values as $value)
                                                            <option value="{{ $value->id }}"
                                                                @php
                                                                    // Loại bỏ khoảng trắng thừa và so sánh
                                                                    $isSelected = in_array(trim($value->value), array_map('trim', $skuList->toArray())) ? 'selected' : '';
                                                                @endphp
                                                                {{ $isSelected }}>
                                                                {{ $value->value }}
                                                            </option>
                                                        ...
                                                        <span class="error" id="attribute_{{ $attribute->id }}-error"></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="box  box-solid box-default" style="margin-bottom: 5px;">
                                    <div class="box-header">
                                        <h3 class="box-title">
                                            ...
                                            <span class="color">{{preg_replace('/^[a-zA-Z0-9]+-/', '', $product_detail->sku)}}</span>
                                        </h3>
                                        <div class="box-tools">
                                            <button class="btn btn-box-tool" data-widget="collapse" data-toggle="tooltip"
                                                title="Collapse"><i class="fa fa-minus"></i></button>
                                            <button class="btn btn-box-tool delete_detail" title="Remove"><i
    ...
    $('select[name^="values"]').each(function() {
        ...
        if (Array.isArray(currentValue)) {
            currentValue.forEach(function(value) {
                ...
                if (!isValueExists && !newAttributes.some(attr => attr.attributeName === attributeName && attr.value === value)) {
                    newAttributes.push({ attributeName: attributeName, value: value });
                }
            });
        } else {
            let isValueExists = false;
            ...
            // Nếu chưa có, thêm vào newAttributes với tên thuộc tính và giá trị
            if (!isValueExists && !newAttributes.some(attr => attr.attributeName === attributeName && attr.value === currentValue)) {
                newAttributes.push({ attributeName: attributeName, value: currentValue });
            }
        }
    });
...
newAttributes.forEach(attr => {
    if (existingAttributes[attr.attributeName]) {
        const otherAttributeName = Object.keys(existingAttributes).find(name => name !== attr.attributeName);
...
combinations = [...newCombinations, ...combinations];
        ...
        combinations.forEach((combination, index) => {
            const variantName = combination.map(attributeValue => {
                const optionText = $(`option[value="${attributeValue}"]`).text(); // Tìm tên của thuộc tính dựa trên giá trị
                return optionText;
            }).join(' - ');
        ...
        allAttributes.forEach(attribute => {
            ...
            existingAttributes[attribute] = [...existingAttributes[attribute], ...newAttributes];
        });
