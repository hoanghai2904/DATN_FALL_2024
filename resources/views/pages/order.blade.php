                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        @if ($data['order']->status == 4 && !$data['order']->is_received)
                            <button class="btn btn-success" onclick="handleReceiveOrder({{ $data['order']->id }})">Đã nhận
                                hàng</button>
                        @endif
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <ul class="list-group order-info">
                            ...
                            <li class="list-group-item"><span>Phương Thức Thanh Toán:</span>
                                {{ $data['order']->payment_method->name ?? 'Chưa xác định' }}</li>
                            <li class="list-group-item">
                                <span>Trạng thái thanh toán:</span>
                                {{ $data['order']->is_paid ? 'Đã thanh toán' : 'Chưa thanh toán' }}
                                @if (!$data['order']->is_paid && $data['order']->payment_method_id != 1)
                                    <form id="payment-form-{{ $data['order']->id }}"
                                        action="{{ route('payment_now', $data['order']->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $data['order']->id }}">
                                    </form>
                                    <button class="btn btn-primary ml-5"
                                        onclick="document.getElementById('payment-form-{{ $data['order']->id }}').submit();">Thanh
                                        toán ngay</button>
                                @endif
                            </li>
                            <li class="list-group-item">
                            ...
                            <li class="list-group-item"><span>Trạng thái nhận hàng:</span>
                ...
                <tbody>
                    ...
                    @foreach ($data['order']->order_details as $key => $order_detail)
                        ...
                        <tr>
                            ...
                            <td class="text-center text-danger">{{ number_format($order_detail->price, 0, ',', '.') }}₫
                            </td>
                            <td class="text-center text-danger">
                                {{ number_format($order_detail->price * $order_detail->quantity, 0, ',', '.') }}₫</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr>
                        ...
                        <th colspan="2" class="text-center text-danger">{{ number_format($totalAmount, 0, ',', '.') }}₫
                        </th>
                    </tr>
                    <tr>
                        <th colspan="5" class="text-end">Phí giao hàng</th>
                        <th colspan="2" class="text-center text-danger">
                            {{ number_format($data['order']?->fee, 0, ',', '.') }}₫</th>
                    </tr>
                    <tr>
                        <th colspan="5" class="text-end">Giảm giá</th>
                        <th colspan="2" class="text-center text-danger">
                            {{ number_format($data['order']?->discount, 0, ',', '.') }}₫</th>
                    </tr>
                    <tr>
                        ...
                        <th colspan="2" class="text-center text-danger">
                            <strong
                                class="final-total">{{ number_format($totalAmount + $data['order']?->fee - $data['order']?->discount, 0, ',', '.') }}₫</strong>
                        </th>
                    </tr>
                </tfoot>
            </table>
