                ...
                @foreach ($coupons as $coupon)
                    <div class="card-item mb-3">
                        <div class="card-body">
                            ...
                            <p class="card-text"><span>Giảm tối đa:</span>
                                {{ number_format($coupon->max_discount_amount, 0, ',', '.') }}</p>
                            <p class="card-text"><span>Đơn hàng tối thiểu</span>:
                                {{ number_format($coupon->min_order_amount, 0, ',', '.') }}</p>
                            <p class="card-text"><span>Thời gian áp dụng</span>: {{ $coupon->start_date }} ~
                                {{ $coupon->end_date }}</p>
                            @if (in_array($coupon->id, $savedCoupons))
                                <label>Đã lưu</label>
                            @else
                                <button class="btn btn-primary save-coupon"
                                    data-coupon-id="{{ $coupon->id }}">Lưu</button>
                            @endif
                        </div>
                    </div>
                @endforeach
